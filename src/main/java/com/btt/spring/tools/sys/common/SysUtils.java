package com.btt.spring.tools.sys.common;
...
public class SysUtils {
	...
	public static final String NO_INTERCEPTOR_PATH = ".*/((login)|(confirm)|(logout)|(mvc)|(app)|(weixin)|(static)|(main)|(websocket)).*"; // 不对匹配该值的访问路径拦截（正则）
	...
	public static String method2PermissionValue(HandlerMethod method) {
		...
		if (!"".equals(anno.name())) {// @RequestMapper注解写了name属性才做权限收集：所以@RequestMapper注解有没有name备注是作为一个权限的标志
			...
			// System.out.println("得到权限 ,例如：user:delete 用户模块的删除权限=>"+permissionValue
			// +"权限说明:"+anno.name());
			// 构造权限对象,三个参数:权限,模块说明,权限说明
			// permission = new
			// SysPermission(permissionValue,namespaceMapping.name(),
			// anno.name());//把权限控制注解@Permission解析为权限控制对象SysPermission,方便插入数据库权限表
		}
	 ...
	 * 
	 * // 反斜杠 /t 间隔 ('/u0009') /n 换行 ('/u000A') /r 回车 ('/u000D') /d 数字 等价于[0-9]
	 * /D 非数字 等价于[^0-9] /s 空白符号 [/t/n/x0B/f/r] /S 非空白符号 [^/t/n/x0B/f/r] /w 单独字符
	 * [a-zA-Z_0-9] /W 非单独字符 [^a-zA-Z_0-9] /f 换页符 /e Escape /b 一个单词的边界 /B
	 * 一个非单词的边界 /G 前一个匹配的结束
	 * 
	 ...
	 * 
	 * 
	 * 加入特定限制条件「[]」 [a-z] 条件限制在小写a to z范围中一个字符 [A-Z] 条件限制在大写A to Z范围中一个字符
	 * [a-zA-Z] 条件限制在小写a to z或大写A to Z范围中一个字符 [0-9] 条件限制在小写0 to 9范围中一个字符
	 * [0-9a-z] 条件限制在小写0 to 9或a to z范围中一个字符 [0-9[a-z]] 条件限制在小写0 to 9或a to
	 * z范围中一个字符(交集)
	 * 
	 * []中加入^后加再次限制条件「[^]」 [^a-z] 条件限制在非小写a to z范围中一个字符 [^A-Z] 条件限制在非大写A to
	 * Z范围中一个字符 [^a-zA-Z] 条件限制在非小写a to z或大写A to Z范围中一个字符 [^0-9] 条件限制在非小写0 to
	 * 9范围中一个字符 [^0-9a-z] 条件限制在非小写0 to 9或a to z范围中一个字符 [^0-9[a-z]] 条件限制在非小写0 to
	 * 9或a to z范围中一个字符(交集)
	 * 
	 ...
	 * 
	 * 限制为连续出现指定次数字符「{a}」 J{2} JJ J{3} JJJ 文字a个以上，并且「{a,}」 J{3,}
	 * JJJ,JJJJ,JJJJJ,???(3次以上J并存) 文字个以上，b个以下「{a,b}」 J{3,5} JJJ或JJJJ或JJJJJ
	 * 两者取一「|」 J|A J或A Java|Hello Java或Hello
	 */
